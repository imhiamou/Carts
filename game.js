...
function resize() {
  ...
  if (isPhone) {
    ...
    const PHONE_ZOOM = 1.2;
    const HEIGHT_STRETCH = 1.35;
    const Y_SHIFT = -120;
...
function handleIntersection(cart, name, x, y) {
  ...
  else if (dir === "down") { cart.vx = 0; cart.vy = cart.speed; }
  else if (dir === "left") { cart.vx = -cart.speed; cart.vy = 0; }
  else if (dir === "right") { cart.vx = cart.speed; cart.vy = 0; }
...
function resize() {
  ...
  if (isPhone) {
    ...
    const PHONE_ZOOM = 1.25;          // zoom in
    const HEIGHT_STRETCH = 1.35;      // stretch vertically
    const Y_SHIFT = -120;             // lift map upward
